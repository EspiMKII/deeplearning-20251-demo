...
class FFC(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size,
        ...
        self.convg2g = module(in_cg, out_cg,
                                         stride, 1 if groups == 1 else groups // 2)
...
class VGGPL(nn.Module):
    def __init__(self, n_layers=30):
        ...
        base_vgg = models.vgg19(weights=weights)

        for param in base_vgg.parameters():
            param.requires_grad = False

        self.model = nn.Sequential(*list(base_vgg.features)[:n_layers])
...
class GeneratorLoss(nn.Module):
    ...
    def masked_l1_loss(self, pred, target, mask, known_weight=1.):
        loss = self.l1_loss(pred, target)
        known_l1 = known_weight * (1 - mask) * loss
        unknown_l1 = (1 - known_weight) * mask * loss
        return (known_l1 + unknown_l1).mean()
    ...
    def perceptual_loss(self, pred, target):
        fake_feats = self.vgg_pl(pred)
        real_feats = self.vgg_pl(target)
        loss = 0.0
        for f_fake, f_real in zip(fake_feats, real_feats):
...
# ckpt_path = "model_v2.ckpt"
# trainer.fit(model, train_dl, valid_dl, ckpt_path=ckpt_path)
# trainer.fit(model, train_dl, valid_dl)

# # Demo
# def run_inference_from_folder(image_folder_path, ckpt_path="model_v2.ckpt", device='cpu', n_records=10):
#     """
...
def run_inference_from_image(image, mask, ckpt_path="model_v2.ckpt", device='cpu', showimage=False):
    """
